...
fn isColorValid(color: i16) -> bool {
    return (0..255).contains(&color);
}

fn areColorsValid(colors: Vec<i16>) -> bool {
    return colors.iter().all(|&c| isColorValid(c));
}
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if areColorsValid(vec![tuple.0, tuple.1, tuple.2]) {
            Ok(Color {
                red: tuple.0 as u8,
                green: tuple.1 as u8,
                blue: tuple.2 as u8,
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() == 3 && areColorsValid(slice.to_vec()) {
            Ok(Color {
                red: slice[0] as u8,
                green: slice[1] as u8,
                blue: slice[2] as u8,
